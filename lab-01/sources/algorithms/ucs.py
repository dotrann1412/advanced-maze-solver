...
def __normal_ucs(graph, starting_point, ending_point):
	...
	while not frontier.empty() and not found:
		...
		if current_point != starting_point:
			set_frontier_color(current_point[1], current_point[0])
...
def __ucs_with_bonus_point(graph, starting_point, ending_point, bonus_points):
	...
	set_path_color(path, bonus_points)
...
def ucs(graph, starting_point, ending_point, mode, bonus_points, intermediate_points, teleport_points, hf=None):
	if mode == AlgorithmsMode.NORMAL:
		...
		cost = __normal_ucs(graph, starting_point, ending_point)
		ending_time_point = datetime.now()
		print('[*][UCS] Normal mode')
	
	elif mode == AlgorithmsMode.BONUS:
		starting_time_point = datetime.now()
		cost = __ucs_with_bonus_point(graph, starting_point, ending_point, bonus_points)
		ending_time_point = datetime.now()
		print('[*][UCS] Bonus mode')

	elif mode == AlgorithmsMode.INTERMEDIATE:
		starting_time_point = datetime.now()
		cost = __ucs_intermediate_point(graph, starting_point, ending_point, intermediate_points)
		ending_time_point = datetime.now()
		print('[*][UCS] Intermediate mode')

	elif mode == AlgorithmsMode.TELEPORT:
		starting_time_point = datetime.now()
		cost = __ucs_with_teleport_point(graph, starting_point, ending_point, teleport_points)
		ending_time_point = datetime.now()
		print('[*][UCS] Teleport mode')
	
	else:
		print('[*][UCS] Unknown mode')
		return None
